...
body{min-height:100vh;background:var(--bg);color:var(--tx);font-family:'Nunito',sans-serif;overflow-x:hidden;overflow-y:auto}
/* HEADER */
...
#btnProf{width:44px;height:44px;border-radius:50%;background:linear-gradient(135deg,var(--or),var(--gd));border:2px solid rgba(255,215,0,.3);font-size:22px;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0}
...
/* XP */
#xp{padding:5px 14px 8px;background:rgba(13,13,22,.7)}
.xr{display:flex;justify-content:space-between;font-size:10px;color:var(--mt);margin-bottom:4px}
...
.cv{font-size:14px;font-weight:900;color:var(--gd);line-height:1}
.cl{font-size:9px;color:var(--mt);margin-top:2px}
/* CLICKER */
#clicker{display:flex;flex-direction:column;align-items:center;padding:10px 14px 6px}
.nr{width:100%;max-width:320px;margin-bottom:12px}
.nt{display:flex;justify-content:space-between;font-size:11px;margin-bottom:4px}
.nl{color:var(--or);font-weight:800;letter-spacing:1px}
.nb{height:7px;background:rgba(255,255,255,.07);border-radius:4px;overflow:hidden;border:1px solid rgba(255,100,0,.15)}
.nf{height:100%;background:linear-gradient(90deg,#ff3c00,var(--or),#ffa500);border-radius:4px;transition:width .12s linear}
.cw{position:relative;display:flex;align-items:center;justify-content:center;margin:2px 0}
.cg{position:absolute;width:210px;height:210px;border-radius:50%;pointer-events:none;background:radial-gradient(circle,rgba(255,200,0,.18) 0%,transparent 70%);animation:gp 2s ease-in-out infinite}
...
#coin{
  ...
  background:
    ...
    linear-gradient(135deg,#f5c518,#e8a800,#ffd700,#cc8800,#e8a800);
...
#coin.ab.tap{animation:bounce .3s ease;transform:none}
#coin.as.tap{animation:spin .4s linear;transform:none}
#coin.ap.tap{animation:pulse .25s ease;transform:none}
#coin.ak.tap{animation:ashake .3s ease;transform:none}
/* FLOATS */
.ft{position:fixed;pointer-events:none;z-index:9999;font-size:20px;font-weight:900;color:var(--gd);text-shadow:0 0 10px rgba(255,200,0,.9);animation:fup .9s ease-out forwards}
...
.upg.ok::before{content:'';position:absolute;inset:0;background:linear-gradient(135deg,rgba(255,100,0,.05),rgba(255,215,0,.05));pointer-events:none}
.upg.mx{opacity:.38}
.ui{width:58px;height:58px;border-radius:14px;flex-shrink:0;font-size:28px;display:flex;align-items:center;justify-content:center;background:rgba(255,215,0,.08);border:1px solid rgba(255,215,0,.14)}
.ub{flex:1;min-width:0}
.un{font-size:14px;font-weight:800;color:var(--tx);margin-bottom:3px}
.ud{font-size:11px;color:var(--mt);line-height:1.35}
.ue{font-size:11px;color:var(--bl);font-weight:700;margin-top:3px}
.ulv{font-size:10px;color:var(--or);font-weight:700;margin-top:3px}
.up{flex-shrink:0;text-align:right;min-width:58px}
.upv{font-size:14px;font-weight:900;color:var(--gd)}
.upv.no{color:var(--rd)}
.upl{font-size:10px;color:var(--mt);margin-top:1px}
/* PRESTIGE */
...
.pt{font-size:20px;font-weight:900;color:var(--gd);margin-bottom:6px}
.pd{font-size:12px;color:var(--mt);line-height:1.6;margin-bottom:16px}
.pb{background:linear-gradient(135deg,var(--or),var(--gd));border:none;border-radius:12px;padding:14px 32px;font-family:'Nunito',sans-serif;font-size:15px;font-weight:900;color:#000;cursor:pointer;box-shadow:0 4px 18px rgba(255,100,0,.4)}
...
.lbml{font-size:11px;color:var(--gn);font-weight:800}
.lbmv{font-size:16px;font-weight:900;color:var(--gd)}
...
.podt{font-size:10px;font-weight:800;letter-spacing:3px;color:var(--mt);text-transform:uppercase;margin-bottom:14px}
...
.lbcp{font-size:14px;font-weight:900;color:var(--gd);flex-shrink:0}
...
.drinfo{flex:1;min-width:0}
.drname{font-size:19px;font-weight:900;color:var(--gd);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
...
.scv{font-size:20px;font-weight:900;color:var(--gd)}
.scl{font-size:10px;color:var(--mt);margin-top:3px}
...
.achn{font-size:14px;font-weight:800}
...
.skp.no{color:var(--rd)}.skp.ac{color:var(--bl);font-size:10px}
...
.task-card{background:var(--c1);border:1px solid rgba(255,215,0,.15);border-radius:16px;padding:16px;display:flex;align-items:center;gap:14px;margin-bottom:10px}
.task-card.done{opacity:.5;pointer-events:none}
.task-ico{font-size:36px;flex-shrink:0}
.task-body{flex:1;min-width:0}
.task-name{font-size:14px;font-weight:800;color:var(--tx);margin-bottom:3px}
.task-desc{font-size:11px;color:var(--mt);line-height:1.35}
.task-rew{font-size:11px;color:var(--gd);font-weight:700;margin-top:4px}
.task-btn{flex-shrink:0;padding:10px 14px;border-radius:10px;background:linear-gradient(135deg,var(--or),var(--gd));border:none;font-family:'Nunito',sans-serif;font-size:12px;font-weight:900;color:#000;cursor:pointer;white-space:nowrap}
.task-done-lbl{flex-shrink:0;font-size:12px;font-weight:800;color:var(--gn)}
/* NOTIF */
...
#apop.on{bottom:18px}
#apopi{font-size:34px}
.apl{font-size:10px;color:var(--gd);letter-spacing:2px;font-weight:800}
.apn{font-size:14px;font-weight:900}
.apr{font-size:11px;color:var(--bl);margin-top:2px}
/* OFFLINE */
...
<div id="chips">
  <div class="chip"><div class="cv" id="cCpc">+1</div><div class="cl">за клик</div></div>
  <div class="chip"><div class="cv" id="cCps">0</div><div class="cl">/ сек</div></div>
  <div class="chip"><div class="cv" id="cMlt">x1.0</div><div class="cl">множитель</div></div>
  <div class="chip"><div class="cv" id="cCrt">0%</div><div class="cl">крит</div></div>
</div>

<div id="clicker">
  <div class="nr">
    <div class="nt"><span class="nl">⚡ ЭНЕРГИЯ</span><span id="nTxt">100/100</span></div>
    <div class="nb"><div class="nf" id="nFill" style="width:100%"></div></div>
  </div>
  <div class="cw">
    <div class="cg" id="cglow"></div>
...
<div class="panel" id="panel-top">
  <div class="lbme" style="margin-top:6px">
    <span class="lbml">📈 МОЙ ДОХОД / ЧАС</span>
    <span class="lbmv" id="lbMe">0</span>
  </div>
...
<div class="panel" id="panel-extra">
  ...
  <div class="task-card" id="taskTg">
    <div class="task-ico">📢</div>
    <div class="task-body">
      <div class="task-name">Подписаться на канал</div>
      <div class="task-desc">Подпишись на официальный Telegram-канал GymClicker</div>
      <div class="task-rew">💰 Награда: 10,000 монет</div>
    </div>
    <button class="task-btn" id="taskTgBtn">ПЕРЕЙТИ</button>
  </div>
</div>
...
<div id="offpop">
  <div class="offmod">
    ...
    <button class="offbtn" id="offClaimBtn">ЗАБРАТЬ 💰</button>
  </div>
</div>
...
<div id="dr">
  <div class="drh">
    <div class="drhead">
      ...
      <div class="drinfo"><div class="drname" id="drName">Игрок</div><div class="drsub">Ур.<span id="drLvl">1</span> · <span id="drLvlN">Новичок</span></div></div>
  ...
  <div class="drtabs">
    <button class="drtab on" data-p="stats">📊 Статистика</button>
    <button class="drtab" data-p="achs">🏆 Достижения</button>
  ...
  <div class="drbody">
    <div class="drp on" id="drp-stats">
      <div class="sgrid" style="margin-top:2px">
        ...
        <div class="sc"><div class="scv" id="stPR">0</div><div class="scl">Престижей</div></div>
    ...
    <div class="drp" id="drp-skins"><div class="sgd" id="skinList"></div></div>
  </div>
</div>
...
const LEVELS = [
  ...
  {n:'Железный Кулак',x:200000},{n:'Стальная Воля',x:350000},{n:'Гранитный',x:550000},
  {n:'Титановый',x:850000},{n:'Алмазный',x:1300000},{n:'Платиновый',x:2000000},
  {n:'Космический',x:3000000},{n:'Галактический',x:4500000},{n:'Вселенский',x:7000000},
  {n:'Квантовый',x:10000000},{n:'Ультра',x:15000000},{n:'Мега',x:22000000},
  {n:'Гига',x:32000000},{n:'Тера',x:47000000},{n:'Пета',x:70000000},
  {n:'Экза',x:100000000},{n:'ЛЕГЕНДА ВСЕХ ВРЕМЁН',x:150000000},
  {n:'БОГ КАЧАЛКИ',x:220000000},{n:'СОЗДАТЕЛЬ',x:330000000},{n:'БЕСКОНЕЧНЫЙ',x:500000000}
];
...
const UPGRADES = {
  click:[
    ...
    {id:'c8',n:'Перчатки Титана',i:'🧤',d:'Удар с запредельной силой',bp:25000000,pg:2.8,mx:6,ef:'cpc',v:60000},
  ...
  crit:[
    {id:'cr1',n:'Меткость',i:'🎯',d:'Шанс критического удара',bp:800,pg:2.3,mx:5,ef:'critC',v:5},
    {id:'cr2',n:'Снайпер',i:'🔭',d:'Больше шанс крита',bp:25000,pg:2.5,mx:5,ef:'critC',v:10},
  ],
  passive:[
    ...
    {id:'p8',n:'Межгалактическая сеть',i:'🌌',d:'Галактики качаются на тебя',bp:50000000,pg:2.5,mx:8,ef:'cps',v:40000},
    {id:'p9',n:'Вселенская сеть',i:'✨',d:'Вселенная работает на тебя',bp:500000000,pg:2.6,mx:6,ef:'cps',v:400000},
    {id:'p10',n:'Машина времени',i:'⏰',d:'Монеты из прошлого и будущего',bp:5000000000,pg:2.7,mx:5,ef:'cps',v:4000000},
  ],
  auto:[
    {id:'au1',n:'Авто-рука',i:'🤖',d:'Автоматически кликает',bp:10000,pg:9999,mx:1,ef:'aCps',v:1},
    {id:'au2',n:'Дрон-тренер',i:'🚁',d:'Летающий кликер',bp:75000,pg:9999,mx:1,ef:'aCps',v:5},
  ],
  special:[
    {id:'sp1',n:'Комбо-удар',i:'🎰',d:'Каждый 10-й клик даёт x10',bp:50000,pg:9999,mx:1,ef:'combo',v:1},
    {id:'sp2',n:'Фортуна',i:'🍀',d:'15% шанс удвоить монеты',bp:35000,pg:9999,mx:1,ef:'luck',v:15},
  ],
};
const ALL_UPGconst ALL_UPG = Object.values(UPGRADES).flat();
...
const DEF = {
  ...
  pres:0,mult:1,lvl:1,xp:0,nrg:100,mxE:100,rgE:2,
...
let G = Object.assign({}, DEF);

function saveGame() {
  G.lastSeen = Date.now();
  try { localStorage.setItem('gym10', JSON.stringify(G)); } catch(e) {}
}
function loadGame() {
  try {
    const r = localStorage.getItem('gym10');
    if (r) Object.assign(G, JSON.parse(r));
  } catch(e) {}
}
...
function recalc() {
  let cpc=1, cps=0, mxE=100, rgE=2, critC=0, critM=2,
      aCps=0, offH=2, luck=0, combo=0, chain=0, gm=1;
  for (const u of ALL_UPG) {
    const l = uLvl(u.id);
    if (!l) continue;
    switch(u.ef) {
      case 'cpc':   cpc   += u.v * l; break;
      case 'cps':   cps   += u.v * l; break;
      case 'mxE':   mxE   += u.v * l; break;
      case 'rgE':   rgE   += u.v * l; break;  // +0.3 per level
      case 'critC': critC += u.v * l; break;
      case 'critM': critM += u.v * l; break;

      case 'aCps':  aCps  += u.v * l; break;
      case 'offH':  offH  += u.v * l; break;
      case 'luck':  luck  += u.v * l; break;
      case 'combo': combo += u.v * l; break;
    }
  }
...
function checkAchs() {
  const s = {clicks:G.clicks, allC:G.allC, allU:G.allU, cps:G.cps,
             pres:G.pres, lvl:G.lvl, crits:G.crits, combos:G.combos,
             ul:G.ul, sk:G.sk};
  for (const a of ACHS) {
    if (G.achs.includes(a.id) || !a.c(s)) continue;
    G.achs.push(a.id);
    if (a.r.t === 's' && !G.sk.includes(a.r.v)) {
      G.sk.push(a.r.v);
      G.claimed.push(a.id);
    }
    showAchPopup(a);
...
function claimReward(id) {
  ...
  if (a.r.t === 'c') {
    ...
    G.allC  += a.r.v;
    updateHUD();
    showNotif('💰 +' + fmt(a.r.v) + ' монет!');
  } else if (a.r.t === 's' && !G.sk.includes(a.r.v)) {
    G.sk.push(a.r.v);
    showNotif('🎨 Скин разблокирован!');
    renderSkins();
  }
  saveGame();
  renderAchs();
...
function doClick(x, y) {
  ...
  if (G.combo > 0) {
    G.comboN++;
    if (G.comboN >= 10) {
      earn *= 10; G.comboN = 0; G.combos++;
      spawnFloat('КОМБО x10!', x, y - 30);
    }
  }
  if (G.critC > 0 && Math.random() * 100 < G.critC) {
    earn = Math.floor(earn * G.critM);
    isCrit = true; G.crits++;
  }
  if (G.luck > 0 && Math.random() * 100 < G.luck) {
    earn *= 2;
    spawnFloat('УДАЧА x2!', x, y - 30);
  }
  ...
  checkLevel();
  checkAchs();
  spawnFloat((isCrit ? '💥 ' : '+') + fmt(earn), x, y, isCrit ? '#ff5500' : null);
  spawnRipple(x, y);
  updateHUD();
}
...
function buyUpgrade(id) {
  ...
  G.xp += 10;
  if (u.ef === 'tb') {
    tbActive = true; tbMult = u.v;
    setTimeout(() => { tbActive = false; }, 30000);
    showNotif('🚀 x' + u.v + ' монеты 30 сек!');
  }
  recalc(); checkLevel(); checkAchs(); updateHUD(); renderAll();
  if (u.ef !== 'tb') showNotif('✅ ' + u.n + ' — Ур.' + (l+1));
}

function tapSkin(id) {
  const s = SKINS.find(x => x.id === id);
  if (!s) return;
  if (s.ach && !G.sk.includes(id)) { showNotif('🔒 Нужно достижение!'); return; }
  if (G.sk.includes(id)) {
    G.skin = id; applySkin(id); saveGame(); renderSkins();
    showNotif('✅ Скин надет!');
  } else {
    if (G.coins < s.p) { showNotif('💸 Недостаточно монет!'); return; }
    G.coins -= s.p; G.sk.push(id); G.skin = id;
    applySkin(id); saveGame(); updateHUD(); renderSkins();
...
function applySkin(id) {
  ...
  btn.style.boxShadow   = s.sh;
  btn.textContent       = s.e;
  btn.className = 'ab as ap ak'.split(' ')
    .reduce((c, a) => c.replace(a, ''), btn.className).trim();
  if (s.an) btn.classList.add(s.an);
  if (glow) glow.style.background =
    'radial-gradient(circle,' + s.gl + ' 0%,transparent 70%)';
...
function updateHUD() {
  ...
  const ci = G.lvl - 1;
  const cx = LEVELS[ci]?.x || 0;
  const nx = LEVELS[G.lvl]?.x ?? (LEVELS[LEVELS.length-1].x + 999999);
  ...
  document.getElementById('lvlName').textContent = LEVELS[G.lvl-1]?.n || 'АБСОЛЮТ';
  ...
  document.getElementById('stCR').textContent = fmt(G.crits);
...
function effText(u) {
  const v = u.v;
  switch(u.ef) {
    case 'cpc':   return '+' + v + ' за клик';
    case 'cps':   return '+' + v + '/сек · +' + fmt(v * 3600) + '/ч';
    case 'mxE':   return '+' + v + ' энергии';
    case 'rgE':   return '+' + v.toFixed(1) + ' восст./сек';
    case 'critC': return '+' + v + '% крит шанс';
    case 'critM': return '+' + v + 'x крит урон';
    case 'gm':    return '+' + (v * 100) + '% к монетам';
    case 'aCps':  return '+' + v + ' авто/сек';
    case 'offH':  return '+' + v + 'ч оффлайна';
    case 'luck':  return '+' + v + '% удвоить';
    case 'combo': return 'каждый 10-й клик x10';
    case 'chain': return 'каждый клик подряд +1';
    case 'tb':    return 'x' + v + ' монеты на 30 сек';
    default:      return '';
  }
}
...
function renderList(cid, list) {
  ...
  for (const u of list) {
    ...
    const mx = l >= u.mx;
    const p  = uPrice(u);
    const ok = !mx && G.coins >= p;
    const cls = 'upg' + (ok ? ' ok' : '') + (mx ? ' mx' : '');
    const priceHtml = mx
      ? '<div class="upv" style="color:var(--gn)">МАКС</div>'
      : '<div class="upv' + (ok ? '' : ' no') + '">' + fmt(p) + '</div><div class="upl">монет</div>';
    parts.push(
      '<div class="' + cls + '" data-uid="' + u.id + '">' +
        '<div class="ui">' + u.i + '</div>' +
        '<div class="ub">' +
          '<div class="un">' + u.n + '</div>' +
          '<div class="ud">' + u.d + '</div>' +
          '<div class="ue">' + effText(u) + '</div>' +
          '<div class="ulv">Ур. ' + l + ' / ' + u.mx + '</div>' +
        '</div>' +
        '<div class="up">' + priceHtml + '</div>' +
      '</div>'
    );
  }
...
function renderAll() {
  renderList('lClick',   UPGRADES.click);
  renderList('lEnergy',  UPGRADES.energy);
  renderList('lCrit',    UPGRADES.crit);
  renderList('lPassive', UPGRADES.passive);
  renderList('lAuto',    UPGRADES.auto);
  renderList('lSpecial', UPGRADES.special);
}
...
function renderAchs() {
  ...
  for (const a of ACHS) {
    ...
    const cl   = G.claimed.includes(a.id);
    let rv = a.r.t === 'c' ? '💰 ' + fmt(a.r.v) + ' монет' : '';
    if (a.r.t === 's') {
      const sk = SKINS.find(x => x.id === a.r.v);
      rv = '🎨 ' + (sk ? sk.n : 'скин');
    }
    let right = '';
    if (done && !cl) right = '<button class="achbtn" data-aid="' + a.id + '">ЗАБРАТЬ</button>';
    else if (done && cl) right = '<div class="achgot">✓</div>';
    parts.push(
      '<div class="ach' + (done ? ' on' : '') + '">' +
        '<div class="achi">' + a.i + '</div>' +
        '<div class="achb">' +
          '<div class="achn">' + a.n + '</div>' +
          '<div class="achd">' + (done ? a.d : '???') + '</div>' +
          '<div class="achr">' + rv + '</div>' +
        '</div>' +
        (done ? '<div>' + right + '</div>' : '') +
      '</div>'
    );
...
function renderSkins() {
  ...
  for (const s of SKINS) {
    const own  = G.sk.includes(s.id);
    const eq   = G.skin === s.id;
    const alck = s.ach && !own;
    const ok   = !own && !alck && G.coins >= s.p;
    let badge = '';
    ...
    let price = '';
    if (s.p === 0 && !alck)
      price = '<div class="skp" style="color:var(--gn)">БЕСПЛАТНО</div>';
    else if (alck) {
      const a = ACHS.find(x => x.id === s.ach);
      price = '<div class="skp ac">' + (a ? a.n : 'Достижение') + '</div>';
    } else if (own)
      price = eq
        ? '<div class="skp" style="color:var(--gn)">НАДЕТ</div>'
        : '<div class="skp" style="color:var(--or)">НАДЕТЬ</div>';
    else
      price = '<div class="skp' + (ok ? '' : ' no') + '">' + fmt(s.p) + ' 💰</div>';
    const cls = 'sk' + (eq?' eq':own?' own':alck?' ak':ok?' ok':' lk');
    parts.push(
      '<div class="' + cls + '" data-sid="' + s.id + '">' +
        badge +
        '<div class="skprev" style="background:' + s.bg + ';box-shadow:' + s.sh + '">' + s.e + '</div>' +
        '<div class="skn">' + s.n + '</div>' +
        price +
      '</div>'
    );
  }
...
function showNotif(msg) {
  ...
  notifTimer = setTimeout(() => el.classList.remove('on'), 2200);
}

let achQueue = [], achBusy = false;
function showAchPopup(a) { achQueue.push(a); if (!achBusy) nextAch(); }
function nextAch() {
  if (!achQueue.length) { achBusy = false; return; }
  achBusy = true;
  const a = achQueue.shift();
  document.getElementById('apopi').textContent = a.i;
  document.getElementById('apN').textContent   = a.n;
  let rw = a.r.t === 'c' ? 'Награда: 💰' + fmt(a.r.v) : '';
  if (a.r.t === 's') {
    const sk = SKINS.find(x => x.id === a.r.v);
    rw = 'Награда: 🎨 ' + (sk ? sk.n : 'скин');
  }
...
function openDrawer() {
  ...
  const n = getNick();
  if (n) document.getElementById('nickEdit').value = n;
}
...
function changeNick() {
  ...
  updateHUD(); saveGame(); showNotif('✅ Ник: ' + v); pushScore();
}
...
async function pushScore() {
  ...
  try {
    await fetch(API + '/api/score', {
      ...
      body: JSON.stringify({user_id: getId(), username: nick,
                            cph: Math.floor(G.cps * 3600), prestiges: G.pres})
    });
  } catch(e) {}
}
...
function buildPodium(rows) {
  ...
  const num = t3.length >= 2 ? [2, 1, 3] : [1];
  let html = '<div class="pod"><div class="podt">🏆 Зал Славы</div><div class="pods">';
  ord.forEach((r, i) => {
    const me = r.username === myN;
    html +=
      '<div class="podsl ' + cls[i] + '">' +
        '<div class="podc">' + (num[i] === 1 ? '👑' : '') + '</div>' +
        '<div class="podci">💪</div>' +
        '<div class="podnm"' + (me ? ' style="color:var(--gn)"' : '') + '>' +
          r.username + (me ? ' 👈' : '') +
        '</div>' +
        '<div class="podcp">' + fmt(r.cph) + '/ч</div>' +
        '<div class="podbk">' + num[i] + '</div>' +
      '</div>';
  });
  html += '</div></div>';
  el.innerHTML = html;
...
async function loadLB() {
  ...
  el.innerHTML = '<div class="lbem">Загрузка...</div>';
  ...
  try {
    ...
    const myN = getNick();
    let html = '<div class="poddiv"></div>';
    rest.forEach((r, i) => {
      const me = r.username === myN;
      html +=
        '<div class="lbrow' + (me ? ' me' : '') + '">' +
          '<div class="lbrk">' + (i + 4) + '</div>' +
          '<div class="lbnm">' + r.username + (me ? ' 👈' : '') + '</div>' +
          '<div style="text-align:right;flex-shrink:0">' +
            '<div class="lbcp">' + fmt(r.cph) + '/ч</div>' +
    ...
    document.getElementById('lbPod').innerHTML = '';
    el.innerHTML = '<div class="lbem">Ошибка загрузки</div>';
  }
}
...
window.addEventListener('DOMContentLoaded', function() {
  ...
  coin.addEventListener('touchstart', function(e) {
    ...
    for (let i = 0; i < e.changedTouches.length; i++) {
      doClick(e.changedTouches[i].clientX, e.changedTouches[i].clientY);
    }
  }, {passive: false});
  ...
  // PROFILE
  document.getElementById('btnProf').addEventListener('click', openDrawer);
  ...
  document.getElementById('nickIn').addEventListener('keydown', e => {
    if (e.key === 'Enter') saveNick();
  });
  document.getElementById('nickEditBtn').addEventListener('click', changeNick);

  // OFFLINE
  document.getElementById('offClaimBtn').addEventListener('click', claimOffline);

  // LB REFRESH
  document.getElementById('lbRefBtn').addEventListener('click', loadLB);

  // EXTRA TAB tasks
  const taskTgBtn = document.getElementById('taskTgBtn');
  if (taskTgBtn) {
    const taskDone = localStorage.getItem('task_tg_done');
    if (taskDone) {
      taskTgBtn.textContent = '✓ ВЫПОЛНЕНО';
      taskTgBtn.disabled = true;
      document.getElementById('taskTg').classList.add('done');
    }
    taskTgBtn.addEventListener('click', function() {
      window.open('https://t.me/gymclicker', '_blank');
      setTimeout(() => {
        if (!localStorage.getItem('task_tg_done')) {
          localStorage.setItem('task_tg_done', '1');
          G.coins += 10000; G.allC += 10000;
          updateHUD();
          showNotif('💰 +10,000 монет за подписку!');
          taskTgBtn.textContent = '✓ ВЫПОЛНЕНО';
          taskTgBtn.disabled = true;
          document.getElementById('taskTg').classList.add('done');
          saveGame();
  ...
  document.getElementById('tabs').addEventListener('click', function(e) {
    ...
    if (t === 'top') loadLB();
    if (t === 'extra') {
      // restore task button state
      const taskDone = localStorage.getItem('task_tg_done');
      const btn = document.getElementById('taskTgBtn');
      if (btn && taskDone) {
  ...
  // PROFILE TABS
  document.querySelectorAll('.drtab').forEach(b => {
    b.addEventListener('click', function() {
      const p = this.dataset.p;
      document.querySelectorAll('.drtab').forEach(x => x.classList.toggle('on', x.dataset.p === p));
      document.querySelectorAll('.drp').forEach(x => x.classList.toggle('on', x.id === 'drp-' + p));
      if (p === 'achs')  renderAchs();
      if (p === 'skins') renderSkins();
    });
  });

  // UPGRADES — event delegation on panels
  document.getElementById('tabs').parentElement.addEventListener('click', function(e) {
    const u = e.target.closest('[data-uid]');
    if (u) { buyUpgrade(u.dataset.uid); return; }
    const s = e.target.closest('[data-sid]');
    if (s) { tapSkin(s.dataset.sid); return; }
    const a = e.target.closest('[data-aid]');
    if (a) { claimReward(a.dataset.aid); return; }
  });

  // DRAWER delegation
  document.getElementById('dr').addEventListener('click', function(e) {
    const s = e.target.closest('[data-sid]');
    if (s) { tapSkin(s.dataset.sid); return; }
    const a = e.target.closest('[data-aid]');
    if (a) { claimReward(a.dataset.aid); return; }
