...
<div class="panel" id="panel-passive">
  ...
  <div class="pbox">
    <div class="pt">🔥 ПРЕСТИЖ</div>
    <div class="pd">Сбрось прогресс — получи постоянный множитель.<br>Нужно: <strong id="presReq" style="color:var(--gd)">100,000</strong></div>
    <button class="pb" id="presBtn" disabled>ПРЕСТИЖ</button>
  </div>
</div>
...
const UPGRADES = {
  ...
  crit:[
    {id:'cr1',n:'Меткость',i:'🎯',d:'Шанс критического удара',bp:800,pg:2.3,mx:20,ef:'critC',v:5},
    {id:'cr2',n:'Разрушитель',i:'💣',d:'Множитель критического удара',bp:5000,pg:2.4,mx:15,ef:'critM',v:0.5},
    {id:'cr3',n:'Снайпер',i:'🔭',d:'Больше шанс крита',bp:25000,pg:2.5,mx:10,ef:'critC',v:10},
  ],
  passive:[
  ...
  auto:[
    {id:'au1',n:'Авто-рука',i:'🤖',d:'Автоматически кликает',bp:10000,pg:2.3,mx:20,ef:'aCps',v:1},
  ...
  offline:[
    {id:'of1',n:'Копилка',i:'🐷',d:'Больше времени оффлайн дохода',bp:5000,pg:2.2,mx:10,ef:'offH',v:0.5},
...
const ALL_UPG = Object.values(UPGRADES).flat();
...
function doPrestige() {
  ...
  G.pres++;
  G.mult   = 1 + G.pres * 0.5;
  G.coins  = 0; G.xp = 0; G.lvl = 1;
  G.nrg    = G.mxE;
  G.ul     = {};
  G.chainV = 0; G.comboN = 0;
  recalc(); checkAchs(); updateHUD(); renderAll();
  showNotif('🔥 ПРЕСТИЖ! Множитель x' + G.mult.toFixed(1));
}
...
function renderAll() {
  ...
  renderList('lMulti',   UPGRADES.multi);
  renderList('lAuto',    UPGRADES.auto);
  renderList('lOffline', UPGRADES.offline);
...
window.addEventListener('DOMContentLoaded', function() {
  ...
  // PRESTIGE
  document.getElementById('presBtn').addEventListener('click', doPrestige);
