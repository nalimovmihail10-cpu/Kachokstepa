...
.tabs-arrow{position:absolute;top:50%;transform:translateY(-50%);width:28px;height:28px;border-radius:50%;background:linear-gradient(135deg,var(--or),var(--gd));display:flex;align-items:center;justify-content:center;font-size:12px;color:#000;font-weight:900;cursor:pointer;z-index:10;box-shadow:0 2px 8px rgba(255,100,0,.4);opacity:0;transition:opacity .3s;pointer-events:none}
.tabs-arrow.show{opacity:1;pointer-events:all}
.tabs-arrow.left{left:2px}.tabs-arrow.right{right:2px}
...
.coin-spin{animation:coinspin 2s linear infinite;display:inline-block;transform-origin:center}
@keyframes coinspin{0%{transform:rotateY(0deg)}25%{transform:rotateY(90deg) scaleX(.1)}50%{transform:rotateY(180deg)}75%{transform:rotateY(270deg) scaleX(.1)}100%{transform:rotateY(360deg)}}
...
.reset-btn:active{background:rgba(255,34,68,.2)}
.crash-wrap{padding:10px 14px 100px}
.crash-header{text-align:center;margin-bottom:16px}
.crash-title{font-size:28px;font-weight:900;color:var(--rd);text-shadow:0 0 20px rgba(255,34,68,.5)}
.crash-subtitle{font-size:12px;color:var(--mt);margin-top:4px}
.crash-screen{background:var(--c1);border:1px solid rgba(255,34,68,.2);border-radius:16px;padding:20px;text-align:center;margin-bottom:16px;position:relative;overflow:hidden;min-height:140px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px}
.crash-multiplier{font-size:52px;font-weight:900;color:var(--gd);text-shadow:0 0 30px rgba(255,200,0,.6);line-height:1;transition:color .2s}
.crash-multiplier.danger{color:var(--rd);text-shadow:0 0 30px rgba(255,34,68,.6);animation:shake2 .1s infinite}
@keyframes shake2{0%,100%{transform:translateX(0)}50%{transform:translateX(3px)}}
.crash-coin-wrap{width:100%;max-width:300px}
.crash-coin{font-size:36px;transition:transform .1s;display:inline-block}
.crash-coin.running{animation:coinrun .5s ease-in-out infinite}
@keyframes coinrun{0%,100%{transform:translateY(0) rotate(0)}50%{transform:translateY(-8px) rotate(10deg)}}
.crash-track{height:6px;background:rgba(255,255,255,.07);border-radius:3px;overflow:hidden;margin-top:8px}
.crash-fill{height:100%;background:linear-gradient(90deg,var(--gn),var(--gd));border-radius:3px;transition:width .1s linear}
.crash-fill.danger{background:linear-gradient(90deg,var(--or),var(--rd))}
.crash-status{font-size:13px;color:var(--mt);font-weight:700}
.crash-bet-wrap{background:var(--c1);border:1px solid rgba(255,215,0,.12);border-radius:16px;padding:16px;margin-bottom:14px}
.crash-bet-label{font-size:10px;font-weight:800;letter-spacing:2px;color:var(--mt);text-transform:uppercase;margin-bottom:10px}
.crash-bet-row{display:flex;gap:6px;flex-wrap:wrap;margin-bottom:10px}
.crash-bet-btn{flex:1;min-width:44px;padding:8px 4px;background:var(--c2);border:1px solid rgba(255,215,0,.15);border-radius:9px;color:var(--gd);font-family:'Nunito',sans-serif;font-size:12px;font-weight:800;cursor:pointer}
.crash-bet-btn.sel{background:linear-gradient(135deg,rgba(255,100,0,.2),rgba(255,215,0,.1));border-color:rgba(255,215,0,.4)}
.crash-bet-btn:active{transform:scale(.95)}
.crash-custom-row{display:flex;gap:8px;margin-bottom:10px}
.crash-input{flex:1;padding:9px 12px;border-radius:9px;border:1.5px solid rgba(255,215,0,.2);background:rgba(255,255,255,.06);color:var(--tx);font-family:'Nunito',sans-serif;font-size:14px;font-weight:700;outline:none;-webkit-appearance:none}
.crash-input:focus{border-color:rgba(255,215,0,.5)}
.crash-set-btn{padding:9px 14px;border-radius:9px;background:linear-gradient(135deg,var(--or),var(--gd));border:none;font-family:'Nunito',sans-serif;font-size:13px;font-weight:900;color:#000;cursor:pointer}
.crash-current-bet{font-size:13px;font-weight:800;color:var(--gd);text-align:center}
.crash-btns{display:flex;gap:10px;margin-bottom:16px}
.crash-start-btn,.crash-cash-btn{flex:1;padding:14px;border:none;border-radius:13px;font-family:'Nunito',sans-serif;font-size:15px;font-weight:900;cursor:pointer;transition:transform .1s}
.crash-start-btn{background:linear-gradient(135deg,var(--or),var(--gd));color:#000}
.crash-start-btn:disabled{opacity:.35;cursor:not-allowed}
.crash-cash-btn{background:linear-gradient(135deg,#00aa00,var(--gn));color:#000}
.crash-cash-btn:disabled{opacity:.35;cursor:not-allowed}
.crash-start-btn:active:not(:disabled),.crash-cash-btn:active:not(:disabled){transform:scale(.97)}
.crash-history{background:var(--c1);border:1px solid rgba(255,255,255,.06);border-radius:14px;padding:14px}
.crash-hist-title{font-size:10px;font-weight:800;letter-spacing:2px;color:var(--mt);text-transform:uppercase;margin-bottom:10px}
.crash-hist-list{display:flex;flex-wrap:wrap;gap:6px}
.ch{padding:5px 10px;border-radius:20px;font-size:12px;font-weight:800}
.ch.win{background:rgba(57,255,20,.15);color:var(--gn);border:1px solid rgba(57,255,20,.3)}
.ch.lose{background:rgba(255,34,68,.12);color:var(--rd);border:1px solid rgba(255,34,68,.25)}
...
<div id="tabs-wrap"><div id="tabs">
  ...
  <button class="tab" data-t="crash">&#x1F4A5; CRASH</button>
...
<div class="panel" id="panel-crash">
  <div class="crash-wrap">
    <div class="crash-header">
      ...
      <div class="crash-subtitle">Забери до краша — или потеряй всё</div>
    </div>
    <div class="crash-screen">
      <div class="crash-multiplier" id="crashMult">x1.00</div>
      <div class="crash-coin-wrap">
        <div class="crash-coin" id="crashCoin">💪</div>
        <div class="crash-track"><div class="crash-fill" id="crashFill" style="width:0%"></div></div>
      </div>
      <div class="crash-status" id="crashStatus">Сделай ставку и начни</div>
    </div>
    <div class="crash-bet-wrap">
      <div class="crash-bet-label">Ставка (💪 бицушки)</div>
      <div class="crash-bet-row">
        <button class="crash-bet-btn" data-bet="100">100</button>
        <button class="crash-bet-btn" data-bet="500">500</button>
        <button class="crash-bet-btn" data-bet="1000">1K</button>
        <button class="crash-bet-btn" data-bet="5000">5K</button>
        <button class="crash-bet-btn" data-bet="10000">10K</button>
        <button class="crash-bet-btn" data-bet="all">ВСЕ</button>
      </div>
      <div class="crash-custom-row">
        <input class="crash-input" id="crashBetIn" type="number" placeholder="Своя ставка..." min="1">
        <button class="crash-set-btn" id="crashSetBtn">✓</button>
      </div>
      <div class="crash-current-bet" id="crashBetDisplay">Ставка: 0 💪</div>
    </div>
    <div class="crash-btns">
      <button class="crash-start-btn" id="crashStartBtn">🚀 НАЧАТЬ</button>
      <button class="crash-cash-btn" id="crashCashBtn" disabled>💰 ЗАБРАТЬ</button>
    </div>
    <div class="crash-history">
      <div class="crash-hist-title">История</div>
      <div class="crash-hist-list" id="crashHistory"></div>
    </div>
  </div>
</div>
...
<div id="dr">
  ...
  <div class="drbody">
    <div class="drp on" id="drp-stats">
      ...
      <button class="reset-btn" id="resetBtn">&#x1F5D1; Сбросить мой прогресс</button>
    </div>
...
{id:'c4',n:'Анаболики',i:'💉',d:'Сила зашкаливает',bp:4000,pg:2.4,mx:20,ef:'cpc',v:40},
{id:'c5',n:'Режим зверя',i:'🦁',d:'Ты непобедим',bp:30000,pg:2.5,mx:20,ef:'cpc',v:200},
{id:'c6',n:'Бог качалки',i:'🏛️',d:'Запредельная мощь',bp:300000,pg:2.6,mx:15,ef:'cpc',v:1200},
{id:'c7',n:'Квантовый удар',i:'⚛️',d:'Разрушает пространство',bp:3000000,pg:2.7,mx:12,ef:'cpc',v:8000},
{id:'c8',n:'Перчатки Титана',i:'🧤',d:'Сила запредельная',bp:25000000,pg:2.8,mx:10,ef:'cpc',v:60000},
{id:'c9',n:'Молот Тора',i:'🔨',d:'Мощь бога грома',bp:200000000,pg:2.9,mx:8,ef:'cpc',v:500000},
{id:'c10',n:'Перчатки Бесконечности',i:'♾️',d:'Неограниченная сила',bp:2000000000,pg:3.0,mx:6,ef:'cpc',v:5000000},
{id:'c11',n:'Удар Галактики',i:'🌌',d:'Пробивает галактики',bp:20000000000,pg:3.1,mx:5,ef:'cpc',v:50000000},
{id:'c12',n:'Взрыв Вселенной',i:'💥',d:'Мощь Большого взрыва',bp:200000000000,pg:3.2,mx:4,ef:'cpc',v:500000000},
{id:'c13',n:'Рука Бога',i:'✋',d:'Бог нажимает за тебя',bp:2000000000000,pg:3.3,mx:3,ef:'cpc',v:5000000000},
{id:'c14',n:'Омега Удар',i:'🔱',d:'Превосходит всё',bp:20000000000000,pg:3.4,mx:3,ef:'cpc',v:50000000000},
{id:'c15',n:'Бицепс Времени',i:'⏳',d:'Удары сквозь время',bp:200000000000000,pg:3.5,mx:2,ef:'cpc',v:500000000000},
{id:'c16',n:'Сила Стринг-теории',i:'🧵',d:'Струны Вселенной',bp:2000000000000000,pg:3.6,mx:2,ef:'cpc',v:5000000000000},
{id:'c17',n:'Тёмная Материя',i:'🌑',d:'Сила невидимого мира',bp:20000000000000000,pg:3.7,mx:2,ef:'cpc',v:50000000000000},
{id:'c18',n:'Антиматерия',i:'⚫',d:'Уничтожает всё на пути',bp:200000000000000000,pg:3.8,mx:2,ef:'cpc',v:500000000000000},
{id:'c19',n:'Сингулярность',i:'🕳️',d:'Бесконечная плотность',bp:2000000000000000000,pg:3.9,mx:1,ef:'cpc',v:5000000000000000},
{id:'c20',n:'Абсолютный Удар',i:'☄️',d:'Разрушает саму реальность',bp:20000000000000000000,pg:4.0,mx:1,ef:'cpc',v:50000000000000000}
],
energy:[
...
passive:[
{id:'p1',n:'Россия',i:'🇷🇺',d:'Инвестируй в российские качалки',bp:50,pg:1.8,mx:20,ef:'cps',v:0.5},
{id:'p2',n:'Исландия',i:'🇮🇸',d:'Ледяные залы Рейкьявика',bp:300,pg:1.9,mx:20,ef:'cps',v:2},
{id:'p3',n:'США',i:'🇺🇸',d:'Сеть American Gym',bp:1500,pg:2.0,mx:20,ef:'cps',v:8},
{id:'p4',n:'ОАЭ',i:'🇦🇪',d:'Люксовые залы Дубая',bp:8000,pg:2.1,mx:20,ef:'cps',v:30},
{id:'p5',n:'Япония',i:'🇯🇵',d:'Технологии Japanese Fit',bp:50000,pg:2.2,mx:18,ef:'cps',v:150},
{id:'p6',n:'Германия',i:'🇩🇪',d:'Немецкая инженерия силы',bp:200000,pg:2.3,mx:16,ef:'cps',v:600},
{id:'p7',n:'Великобритания',i:'🇬🇧',d:'Королевские залы Лондона',bp:800000,pg:2.3,mx:15,ef:'cps',v:2500},
{id:'p8',n:'Китай',i:'🇨🇳',d:'Миллиард качающихся',bp:3000000,pg:2.4,mx:12,ef:'cps',v:10000},
{id:'p9',n:'Бразилия',i:'🇧🇷',d:'Пляжные качалки Рио',bp:12000000,pg:2.4,mx:10,ef:'cps',v:40000},
{id:'p10',n:'Европейский союз',i:'🇪🇺',d:'Сеть залов по всей Европе',bp:50000000,pg:2.5,mx:10,ef:'cps',v:180000},
{id:'p11',n:'Австралия',i:'🇦🇺',d:'Залы под звёздным небом',bp:200000000,pg:2.5,mx:8,ef:'cps',v:800000},
{id:'p12',n:'Канада',i:'🇨🇦',d:'Хоккейно-силовые залы',bp:800000000,pg:2.6,mx:7,ef:'cps',v:3500000},
{id:'p13',n:'Индия',i:'🇮🇳',d:'Йога и сила 1.4 млрд',bp:3200000000,pg:2.6,mx:6,ef:'cps',v:15000000},
{id:'p14',n:'Весь мир',i:'🌍',d:'Залы на всех континентах',bp:15000000000,pg:2.7,mx:5,ef:'cps',v:70000000},
{id:'p15',n:'МКС',i:'🚀',d:'Тренировки в невесомости',bp:70000000000,pg:2.7,mx:4,ef:'cps',v:350000000},
{id:'p16',n:'Луна',i:'🌙',d:'Первый зал на Луне',bp:350000000000,pg:2.8,mx:4,ef:'cps',v:2000000000},
{id:'p17',n:'Марс',i:'🔴',d:'Качаешься на красной планете',bp:1500000000000,pg:2.8,mx:3,ef:'cps',v:10000000000},
{id:'p18',n:'Юпитер',i:'🪐',d:'Гравитация качает сама',bp:7000000000000,pg:2.9,mx:3,ef:'cps',v:50000000000},
{id:'p19',n:'Галактика',i:'🌌',d:'Галактические качалки',bp:30000000000000,pg:2.9,mx:2,ef:'cps',v:250000000000},
{id:'p20',n:'Машина времени',i:'⏰',d:'Бицушки из прошлого и будущего',bp:150000000000000,pg:3.0,mx:2,ef:'cps',v:1500000000000}
],
special:[
...
var SKINS=[
{id:'default',n:'Золотая Классика',e:'💪',p:0,ach:null,bg:'radial-gradient(circle at 35% 30%,rgba(255,255,180,.55) 0%,transparent 50%),linear-gradient(135deg,#f5c518,#e8a800,#ffd700,#cc8800,#e8a800)',sh:'0 0 0 4px rgba(255,200,0,.28),0 0 30px rgba(255,175,0,.4)',gl:'rgba(255,200,0,.18)'},
...
{id:'blood',n:'Кровавый Атлет',e:'🩸',p:80000,ach:null,bg:'radial-gradient(circle at 35% 25%,rgba(255,100,100,.5) 0%,transparent 50%),linear-gradient(135deg,#8b0000,#cc0000,#ff0000,#990000)',sh:'0 0 0 4px rgba(200,0,0,.5),0 0 40px rgba(180,0,0,.6)',gl:'rgba(180,0,0,.2)'},
{id:'ocean',n:'Океанский Титан',e:'🌊',p:60000,ach:null,bg:'radial-gradient(circle at 30% 25%,rgba(100,200,255,.5) 0%,transparent 50%),linear-gradient(135deg,#003366,#0066cc,#0099ff,#006699)',sh:'0 0 0 4px rgba(0,100,200,.4),0 0 40px rgba(0,120,255,.5)',gl:'rgba(0,150,255,.2)'},
{id:'forest',n:'Лесной Зверь',e:'🌲',p:45000,ach:null,bg:'radial-gradient(circle at 35% 30%,rgba(100,255,100,.4) 0%,transparent 50%),linear-gradient(135deg,#1a4a00,#2d7a00,#3d9900,#1a5500)',sh:'0 0 0 4px rgba(0,150,0,.4),0 0 35px rgba(0,180,0,.5)',gl:'rgba(0,180,0,.2)'},
...
{id:'neon',n:'Неоновый Призрак',e:'👾',p:120000,ach:null,bg:'radial-gradient(circle at 40% 30%,rgba(0,255,255,.5) 0%,transparent 50%),linear-gradient(135deg,#000033,#003333,#00ffff,#001111)',sh:'0 0 0 4px rgba(0,255,255,.5),0 0 50px rgba(0,255,255,.6)',gl:'rgba(0,255,255,.2)'},
{id:'sunset',n:'Закатный Воин',e:'🌅',p:90000,ach:null,bg:'radial-gradient(circle at 35% 25%,rgba(255,200,100,.5) 0%,transparent 50%),linear-gradient(135deg,#ff6600,#ff9900,#ffcc00,#ff6600)',sh:'0 0 0 4px rgba(255,150,0,.4),0 0 40px rgba(255,120,0,.5)',gl:'rgba(255,150,0,.2)'},
{id:'platinum',n:'Платиновый Бог',e:'⚡',p:500000,ach:null,bg:'radial-gradient(circle at 30% 20%,rgba(220,220,255,.8) 0%,transparent 40%),linear-gradient(135deg,#c0c0c0,#e8e8e8,#f5f5f5,#a0a0a0)',sh:'0 0 0 4px rgba(192,192,192,.6),0 0 50px rgba(200,200,255,.7)',gl:'rgba(200,200,220,.3)'},
{id:'inferno',n:'Инферно',e:'😈',p:1000000,ach:null,bg:'radial-gradient(circle at 35% 25%,rgba(255,50,0,.6) 0%,transparent 50%),linear-gradient(135deg,#1a0000,#660000,#ff2200,#330000)',sh:'0 0 0 4px rgba(255,0,0,.5),0 0 60px rgba(200,0,0,.7)',gl:'rgba(200,0,0,.25)'},
{id:'angel',n:'Ангел Силы',e:'👼',p:750000,ach:null,bg:'radial-gradient(circle at 35% 25%,rgba(255,255,255,.9) 0%,transparent 50%),linear-gradient(135deg,#fffff0,#f0f8ff,#fff5ee,#fffacd)',sh:'0 0 0 4px rgba(255,255,200,.6),0 0 50px rgba(255,255,180,.8)',gl:'rgba(255,255,200,.4)'},
{id:'matrix',n:'Матрица',e:'🟩',p:300000,ach:null,bg:'radial-gradient(circle at 40% 30%,rgba(0,255,0,.4) 0%,transparent 50%),linear-gradient(135deg,#000800,#001a00,#003300,#000d00)',sh:'0 0 0 4px rgba(0,255,0,.4),0 0 40px rgba(0,200,0,.6)',gl:'rgba(0,200,0,.2)'}
];];
var SAVE_KEY='gymv11';
var G={};
...
function resetProgress(){
  if(!confirm('Сбросить весь мой прогресс? Это необратимо!'))return;
  localStorage.removeItem(SAVE_KEY);
  localStorage.removeItem(SAVE_KEY+'_tmr');
  TIMERS={};
  G=Object.assign({},DEF);
...
function getSkinEmoji(skinId) {
  for(var i=0;i<SKINS.length;i++){if(SKINS[i].id===skinId)return SKINS[i].e;}
  return '💪';
}
...
function recalc(){
  var cpc=1,cps=0,mxE=100,rgE=2,critC=0,critM=2,luck=0,combo=0;
  // Each level gives 10% more than previous: total = sum(v * 1.10^i) for i=0..l-1
  for(var i=0;i<ALL_UPG.length;i++){
    ...
    if(u.ef==='cpc')cpc+=tot;else if(u.ef==='cps')cps+=tot;
    else if(u.ef==='mxE')mxE+=tot;else if(u.ef==='rgE')rgE+=tot;
    else if(u.ef==='critC')critC+=tot;
    else if(u.ef==='luck')luck+=tot;
    else if(u.ef==='combo')combo+=Math.round(tot);}
...
function tapSkin(id){
  ...
  else{if(G.coins<s.p){showNotif('Недостаточно бицушек!');return;}
...
function nextLvlVal(u) {
  var l = uLvl(u.id);
  return parseFloat((u.v * Math.pow(1.10, l)).toFixed(2));
}
function effTextNext(u) {
  var v = nextLvlVal(u);
...
function renderList(cid,list){
  ...
  for(var i=0;i<list.length;i++){
    ...
    rows.push('<div class="'+cls+'" data-uid="'+u.id+'" style="position:relative"><div class="uico">'+u.i+'</div><div class="ubod"><div class="unam">'+u.n+'</div><div class="udsc">'+u.d+'</div><div class="ueff">'+effTextNext(u)+'</div><div class="ulvl">Ур. '+l+' / '+u.mx+'</div>'+timerHtml+'</div><div class="uprc">'+priceHtml+'</div></div>');}
...
function claimOffline(){G.coins+=_offE;G.allC+=_offE;checkAchs();updateHUD();document.getElementById('offpop').classList.remove('on');showNotif('+'+fmt(_offE)+' бицушек!');}
...
function loadLB(){var el=document.getElementById('lbList');el.innerHTML='<div class="lbem">Загрузка...</div>';document.getElementById('lbPod').innerHTML='';fetch(API+'/api/leaderboard').then(function(r){return r.json();}).then(function(rows){buildPodium(rows);var rest=rows.slice(3);if(!rest.length){el.innerHTML='';return;}var myN=getNick();var html='<div class="poddiv"></div>';for(var i=0;i<rest.length;i++){var r=rest[i];var me=r.username===myN;html+='<div class="lbrow'+(me?' me':'')+'"><div class="lbrk">'+(i+4)+'</div><div style="font-size:18px;flex-shrink:0">'+getSkinEmoji(r.skin)+'</div><div class="lbnm">'+r.username+(me?' &#x1F448;':'')+'</div><div class="lbcp">'+fmt(r.cph)+'/ч</div></div>';}el.innerHTML=html;}).catch(function(){el.innerHTML='<div class="lbem">Ошибка загрузки</div>';});}
...
window.addEventListener('DOMContentLoaded',function(){
  ...
  var rb=document.getElementById('resetBtn');if(rb)rb.addEventListener('click',resetProgress);

  // ── TABS SCROLL ARROWS ──
  (function(){
    var tabs=document.getElementById('tabs');
    var lArr=document.createElement('button');lArr.className='tabs-arrow left';lArr.textContent='<';
    var rArr=document.createElement('button');rArr.className='tabs-arrow right';rArr.textContent='>';
    var wrap=document.getElementById('tabs-wrap');wrap.style.position='relative';
    wrap.appendChild(lArr);wrap.appendChild(rArr);
    function upd(){
      lArr.classList.toggle('show',tabs.scrollLeft>10);
      rArr.classList.toggle('show',tabs.scrollLeft<tabs.scrollWidth-tabs.clientWidth-10);
    }
    tabs.addEventListener('scroll',upd);upd();
    lArr.addEventListener('click',function(){tabs.scrollBy({left:-120,behavior:'smooth'});});
    rArr.addEventListener('click',function(){tabs.scrollBy({left:120,behavior:'smooth'});});
  })();

  // ── CRASH GAME ──
  var CRASH={
    bet:0, running:false, mult:1.0, timer:null, crashAt:1.0,
    history:JSON.parse(localStorage.getItem('crashHist')||'[]')
  };

  function crashGenPoint(){
    // Generate crash point: weighted towards low values
    var r=Math.random();
    if(r<0.40)return 1.0+Math.random()*0.5;  // 40% crash at 1.0-1.5x
    if(r<0.65)return 1.5+Math.random()*1.0;  // 25% crash at 1.5-2.5x
    if(r<0.80)return 2.5+Math.random()*2.5;  // 15% crash at 2.5-5x
    if(r<0.92)return 5+Math.random()*10;      // 12% crash at 5-15x
    if(r<0.98)return 15+Math.random()*35;     // 6%  crash at 15-50x
    return 50+Math.random()*150;              // 2%  crash at 50-200x
  }
  ...
  function setCrashBet(val){
    ...
    b=Math.min(b,Math.floor(G.coins));
    CRASH.bet=b;
    document.getElementById('crashBetDisplay').textContent='Ставка: '+fmt(b)+' 💪';
    document.querySelectorAll('.crash-bet-btn').forEach(function(btn){
  ...
  function crashStart(){
    if(CRASH.running)return;
    if(CRASH.bet<=0){showNotif('Сделай ставку!');return;}
    if(G.coins<CRASH.bet){showNotif('Недостаточно бицушек!');return;}
    G.coins-=CRASH.bet;updateHUD();
    CRASH.running=true;CRASH.mult=1.0;
    CRASH.crashAt=parseFloat(crashGenPoint().toFixed(2));
    var coin=document.getElementById('crashCoin');
    coin.classList.add('running');
    document.getElementById('crashStartBtn').disabled=true;
    document.getElementById('crashCashBtn').disabled=false;
    document.getElementById('crashStatus').textContent='Монетка бежит! Забери вовремя!';
    var speed=0.02;
    CRASH.timer=setInterval(function(){
      CRASH.mult=parseFloat((CRASH.mult+speed).toFixed(2));
      speed=Math.min(speed+0.001,0.15);
      var pct=Math.min(100,(CRASH.mult/CRASH.crashAt)*100);
      var el=document.getElementById('crashMult');
      el.textContent='x'+CRASH.mult.toFixed(2);
      document.getElementById('crashFill').style.width=pct+'%';
      var danger=CRASH.mult>CRASH.crashAt*0.75;
      el.classList.toggle('danger',danger);
      document.getElementById('crashFill').classList.toggle('danger',danger);
  ...
  function crashEnd(won){
    ...
    var coin=document.getElementById('crashCoin');coin.classList.remove('running');
    document.getElementById('crashStartBtn').disabled=false;
    document.getElementById('crashCashBtn').disabled=true;
    if(won){
      var win=Math.floor(CRASH.bet*CRASH.mult);
      G.coins+=win;G.allC+=win;updateHUD();
      CRASH.history.unshift({m:CRASH.mult.toFixed(2),w:true});
      document.getElementById('crashStatus').textContent='Выиграл! +'+fmt(win)+' 💪 (x'+CRASH.mult.toFixed(2)+')';
      ...
      document.getElementById('crashStatus').textContent='КРАШ на x'+CRASH.crashAt+'! Потерял '+fmt(CRASH.bet)+' 💪';
      showNotif('💥 Краш на x'+CRASH.crashAt+'!');
      document.getElementById('crashCoin').textContent='💀';
      document.getElementById('crashMult').style.color='var(--rd)';
      setTimeout(function(){document.getElementById('crashCoin').textContent=G.skin?getSkinEmoji(G.skin):'💪';},1500);
    }
    if(CRASH.history.length>20)CRASH.history=CRASH.history.slice(0,20);
    localStorage.setItem('crashHist',JSON.stringify(CRASH.history));
    renderCrashHistory();
    setTimeout(function(){
      CRASH.mult=1.0;document.getElementById('crashMult').textContent='x1.00';
      document.getElementById('crashMult').style.color='';
      document.getElementById('crashFill').style.width='0%';
      document.getElementById('crashFill').classList.remove('danger');
      document.getElementById('crashMult').classList.remove('danger');
    },2000);
  }

  function renderCrashHistory(){
    var el=document.getElementById('crashHistory');if(!el)return;
    el.innerHTML=CRASH.history.map(function(h){
      return '<span class="ch '+(h.w?'win':'lose')+'">x'+h.m+'</span>';
    }).join('');
  }
  ...
  document.getElementById('crashSetBtn').addEventListener('click',function(){
    var v=parseInt(document.getElementById('crashBetIn').value);
    if(v>0)setCrashBet(v);
  });
  ...
  if(taskBtn){if(localStorage.getItem('task_tg')){taskBtn.textContent='Выполнено';taskBtn.disabled=true;document.getElementById('taskTg').classList.add('done');}
    taskBtn.addEventListener('click',function(){window.open('https://t.me/gymclicker','_blank');var btn=this;setTimeout(function(){if(!localStorage.getItem('task_tg')){localStorage.setItem('task_tg','1');G.coins+=10000;G.allC+=10000;updateHUD();saveGame();showNotif('+10,000 бицушек за подписку!');btn.textContent='Выполнено';btn.disabled=true;document.getElementById('taskTg').classList.add('done');}},3000);});}
  // Set crash coin to current skin
  var cc=document.getElementById('crashCoin');
  if(cc)cc.textContent=getSkinEmoji(G.skin||'default');
