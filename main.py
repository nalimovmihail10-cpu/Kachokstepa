...
function loadGame(){try{var s=localStorage.getItem(SAVE_KEY);G=Object.assign({},DEF,s?JSON.parse(s):{});}catch(e){G=Object.assign({},DEF);}}
function saveGame(){G.lastSeen=Date.now();try{localStorage.setItem(SAVE_KEY,JSON.stringify(G));}catch(e){}}
...
function buyUpgrade(id){
  ...
  if(!u)return;var l=uLvl(id);if(l>=u.mx)return;var p=uPrice(u);
  if(G.coins<p){showNotif('Недостаточно монет!');return;}
  G.coins-=p;G.ul[id]=l+1;G.allU++;G.xp+=10;
  recalc();checkLevel();checkAchs();updateHUD();renderAll();showNotif(u.n+' Ур.'+(l+1));}
function tapSkin(id){
...
function renderList(cid,list){
  ...
  for(var i=0;i<list.length;i++){var u=list[i];var l=uLvl(u.id);var mx=l>=u.mx;var p=uPrice(u);var ok=!mx&&G.coins>=p;
    var cls='upg'+(ok?' ok':'')+(mx?' mx':'');
    var priceHtml=mx?'<div class="uprv" style="color:var(--gn)">МАКС</div>':'<div class="uprv'+(ok?'':' no')+'">'+fmt(p)+'</div><div class="uprl">монет</div>';
    rows.push('<div class="'+cls+'" data-uid="'+u.id+'"><div class="uico">'+u.i+'</div><div class="ubod"><div class="unam">'+u.n+'</div><div class="udsc">'+u.d+'</div><div class="ueff">'+effText(u)+'</div><div class="ulvl">Ур. '+l+' / '+u.mx+'</div></div><div class="uprc">'+priceHtml+'</div></div>');}
...
window.addEventListener('DOMContentLoaded',function(){
  ...
  setInterval(tick,100);setInterval(saveGame,5000);setInterval(renderAll,2000);setInterval(checkAchs,3000);setInterval(pushScore,30000);
