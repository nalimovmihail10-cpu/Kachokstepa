...
.coinwrap{position:relative;display:flex;align-items:center;justify-content:center;margin:2px 0;perspective:400px}
...
#coin.tap{animation:coin3d .25s cubic-bezier(.25,.46,.45,.94);transform:none}
#coin.dead{filter:grayscale(.7) brightness(.5)}
@keyframes coin3d{
  0%  {transform:scale(1) rotateX(0deg) rotateY(0deg)}
  20% {transform:scale(.82) rotateX(15deg) rotateY(-10deg)}
  50% {transform:scale(.88) rotateX(-8deg) rotateY(8deg)}
  75% {transform:scale(1.04) rotateX(4deg) rotateY(-4deg)}
...
.panel{display:none;padding:10px 14px 90px}.panel.on{display:block}
...
#bnav{position:fixed;bottom:0;left:0;right:0;z-index:90;background:rgba(13,13,22,.97);border-top:1px solid rgba(255,215,0,.1);display:flex;padding:0 0 env(safe-area-inset-bottom,0)}
.bnb{flex:1;background:transparent;border:none;color:var(--mt);font-family:'Nunito',sans-serif;cursor:pointer;padding:10px 4px 8px;display:flex;flex-direction:column;align-items:center;gap:3px;transition:color .2s}
.bnb.on{color:var(--gd)}
...
{id:'a30',i:'🖼',n:'Коллекционер фонов',d:'Купи любой фон',c:function(s){return s.bgs&&s.bgs.length>1;},r:{t:'c',v:50000}}
];
...
window.addEventListener('DOMContentLoaded',function(){
  ...
  coin.addEventListener('touchstart',function(e){e.preventDefault();this.classList.add('tap');var self=this;setTimeout(function(){self.classList.remove('tap');},260);for(var i=0;i<e.changedTouches.length;i++)doClick(e.changedTouches[i].clientX,e.changedTouches[i].clientY);},{passive:false});
  coin.addEventListener('mousedown',function(e){this.classList.add('tap');var self=this;setTimeout(function(){self.classList.remove('tap');},260);doClick(e.clientX,e.clientY);});
