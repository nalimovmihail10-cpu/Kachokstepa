...
function renderList(cid,list){
  ...
  el.innerHTML=list.map(u=>{
    const l=uL(u.id),mx=l>=u.mx,p=uP(u),ok=!mx&&G.coins>=p;
    return '<div class="upg'+(ok?' ok':'')+(mx?' mx':'')+'" onclick="buyUpg(\\''+u.id+'\\')">'
      +'<div class="upg-i">'+u.i+'</div>'
      +'<div class="upg-b"><div class="upg-n">'+u.n+'</div>'
        +'<div class="upg-d">'+u.d+'</div>'
        +'<div class="upg-e">'+effText(u)+'</div>'
        +'<div class="upg-l">Ур. '+l+' / '+u.mx+'</div></div>'
      +'<div class="upg-p">'
        +(mx?'<div class="upg-pv" style="color:var(--gn)">МАКС</div>'
            :'<div class="upg-pv'+(ok?'':' no')+'">'+fmt(p)+'</div><div class="upg-pl">монет</div>')
      +'</div></div>';
  }).join('');
}
function renderAll(){
  renderList('lClick',UG.click);renderList('lEnergy',UG.energy);renderList('lCrit',UG.crit);
  renderList('lPassive',UG.passive);renderList('lMulti',UG.multi);
  renderList('lAuto',UG.auto);renderList('lOffline',UG.offline);renderList('lSpecial',UG.special);
}
function renderAchs(){
  ...
  el.innerHTML=ACHS.map(a=>{
    ...
    let right='';
    if(done&&!cl)right='<button class="ach-btn" onclick="claimR(\\''+a.id+'\\')">ЗАБРАТЬ</button>';
    else if(done&&cl)right='<div class="ach-got">✓</div>';
    return '<div class="ach'+(done?' on':'')+'">'
      +'<div class="ach-i">'+a.i+'</div>'
      +'<div class="ach-b"><div class="ach-n">'+a.n+'</div>'
        +'<div class="ach-d">'+(done?a.d:'???')+'</div>'
        +'<div class="ach-r">'+rv+'</div></div>'
      +(done?'<div>'+right+'</div>':'')
      +'</div>';
  }).join('');
}
function renderSkins(){
  ...
  el.innerHTML=SKINS.map(s=>{
    ...
    return '<div class="sk'+(eq?' eq':own?' own':alck?' ak':ok?' ok':' lk')+'" onclick="tapSkin(\\''+s.id+'\\')">'
      +badge+'<div class="sk-prev" style="background:'+s.bg+';box-shadow:'+s.sh+'">'+s.e+'</div>'
      +'<div class="sk-n">'+s.n+'</div>'+price+'</div>';
  }).join('');
}
