...
#clicker{display:flex;flex-direction:column;align-items:center;justify-content:center;padding:10px 14px 6px;width:100%}
...
@keyframes crashShake{0%,100%{transform:translateX(0)}50%{transform:translateX(3px)}}
...
/* SLOTS */
.slots-wrap{padding:10px 14px 110px}
.slots-title{text-align:center;font-size:26px;font-weight:900;color:var(--gd);margin-bottom:3px;text-shadow:0 0 20px rgba(255,200,0,.5);letter-spacing:2px}
.slots-sub{text-align:center;font-size:11px;color:var(--mt);margin-bottom:14px}
.slots-machine{background:linear-gradient(160deg,rgba(30,28,52,1),rgba(18,18,32,1));border:2px solid rgba(255,215,0,.3);border-radius:20px;padding:18px 14px 14px;margin-bottom:12px;box-shadow:0 0 30px rgba(255,200,0,.08),inset 0 1px 0 rgba(255,255,255,.07)}
.slots-reels{display:flex;gap:8px;justify-content:center;margin-bottom:4px}
.slot-reel{width:88px;height:96px;background:rgba(0,0,0,.55);border:2px solid rgba(255,215,0,.2);border-radius:14px;display:flex;align-items:center;justify-content:center;overflow:hidden;position:relative;box-shadow:inset 0 2px 8px rgba(0,0,0,.7)}
.slot-symbol{font-size:50px;line-height:1;display:flex;align-items:center;justify-content:center;width:100%;height:100%;transition:filter .08s}
.slot-reel.spinning .slot-symbol{filter:blur(2px)}
.slot-reel.win{border-color:rgba(57,255,20,.8)!important;box-shadow:0 0 18px rgba(57,255,20,.5),inset 0 2px 8px rgba(0,0,0,.6)!important;animation:sWinPulse .5s ease-in-out infinite}
@keyframes sWinPulse{0%,100%{box-shadow:0 0 10px rgba(57,255,20,.4)}50%{box-shadow:0 0 28px rgba(57,255,20,.9)}}
.slots-payline{height:3px;background:linear-gradient(90deg,transparent,rgba(255,215,0,.5),transparent);margin:8px 0 14px;border-radius:2px}
.slots-bet-wrap{background:rgba(0,0,0,.3);border-radius:14px;padding:14px;margin-bottom:12px;border:1px solid rgba(255,255,255,.05)}
.slots-bet-label{font-size:10px;font-weight:800;letter-spacing:2px;color:var(--mt);text-transform:uppercase;margin-bottom:8px}
.slots-bet-btns{display:flex;gap:5px;flex-wrap:wrap;margin-bottom:8px}
.sbb{flex:1;min-width:38px;padding:8px 4px;background:var(--c2);border:1px solid rgba(255,215,0,.15);border-radius:9px;color:var(--gd);font-family:'Nunito',sans-serif;font-size:11px;font-weight:800;cursor:pointer;transition:transform .1s}
.sbb.sel{background:linear-gradient(135deg,rgba(255,100,0,.2),rgba(255,215,0,.1));border-color:rgba(255,215,0,.4)}
.sbb:active{transform:scale(.94)}
.slots-bet-disp{font-size:13px;font-weight:800;color:var(--gd);text-align:center;margin-top:4px}
.slots-result{text-align:center;min-height:38px;display:flex;align-items:center;justify-content:center;margin-bottom:10px}
.slots-res-txt{font-size:14px;font-weight:900;animation:slotFadeIn .3s ease-out}
@keyframes slotFadeIn{from{opacity:0;transform:scale(.75)}to{opacity:1;transform:scale(1)}}
.slots-spin-btn{width:100%;padding:16px;border:none;border-radius:14px;background:linear-gradient(135deg,var(--or),var(--gd));font-family:'Nunito',sans-serif;font-size:16px;font-weight:900;color:#000;cursor:pointer;box-shadow:0 4px 16px rgba(255,150,0,.4);transition:transform .1s}
.slots-spin-btn:disabled{opacity:.35;cursor:not-allowed;box-shadow:none}
.slots-spin-btn:active:not(:disabled){transform:scale(.97)}
.slots-ctrl{display:flex;gap:8px;margin-bottom:10px}
.slots-vol-btn{flex:0 0 auto;padding:14px 16px;border:1.5px solid rgba(255,255,255,.1);border-radius:14px;background:rgba(255,255,255,.05);color:var(--mt);font-family:'Nunito',sans-serif;font-size:18px;cursor:pointer;transition:all .2s}
.slots-vol-btn.on{color:var(--gd);border-color:rgba(255,215,0,.35);background:rgba(255,215,0,.07)}
.slots-pay-wrap{background:rgba(0,0,0,.3);border-radius:14px;padding:12px 14px;border:1px solid rgba(255,255,255,.05)}
.slots-pay-title{font-size:10px;font-weight:800;letter-spacing:2px;color:var(--mt);text-transform:uppercase;margin-bottom:10px}
.slots-pay-grid{display:grid;grid-template-columns:1fr 1fr;gap:5px}
.spay{display:flex;align-items:center;gap:7px;font-size:11px;padding:5px 8px;background:rgba(255,255,255,.03);border-radius:8px;border:1px solid rgba(255,255,255,.05)}
.spay-sym{font-size:18px;width:22px;text-align:center}
.spay-info{flex:1}.spay-n{font-size:10px;color:var(--mt)}.spay-m{font-weight:900;color:var(--gd);font-size:12px}
.spay-2{grid-column:1/-1;background:rgba(0,200,255,.05);border-color:rgba(0,200,255,.12)}
.spay-2 .spay-m{color:var(--bl)}
...
.mine-stats{display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;padding:12px 14px 4px}
  ...
  <div class="slots-wrap">
    ...
    <div class="slots-machine">
      <div class="slots-reels">
        <div class="slot-reel" id="slotReel1"><div class="slot-symbol" id="slotSym1">&#x1F4AA;</div></div>
        <div class="slot-reel" id="slotReel2"><div class="slot-symbol" id="slotSym2">&#x1F4AA;</div></div>
        <div class="slot-reel" id="slotReel3"><div class="slot-symbol" id="slotSym3">&#x1F4AA;</div></div>
      </div>
      <div class="slots-payline"></div>
      <div class="slots-result" id="slotsResult"><div class="slots-res-txt" style="color:var(--mt)">Нажми КРУТИТЬ!</div></div>
    </div>
    <div class="slots-bet-wrap">
      <div class="slots-bet-label">Ставка (&#x1F4AA; бицушки)</div>
      <div class="slots-bet-btns">
        <button class="sbb" data-sbet="100">100</button>
        <button class="sbb" data-sbet="500">500</button>
        <button class="sbb" data-sbet="1000">1K</button>
        <button class="sbb" data-sbet="5000">5K</button>
        <button class="sbb" data-sbet="10000">10K</button>
        <button class="sbb" data-sbet="all">ВСЕ</button>
      </div>
      <div class="slots-bet-disp" id="slotBetDisp">Ставка: 0 &#x1F4AA;</div>
    </div>
    <div class="slots-ctrl">
      <button class="slots-spin-btn" id="slotSpinBtn">&#x1F3B0; КРУТИТЬ!</button>
      <button class="slots-vol-btn on" id="slotVolBtn">&#x1F50A;</button>
    </div>
    <div class="slots-pay-wrap">
      <div class="slots-pay-title">&#x1F4B0; Таблица выплат</div>
      <div class="slots-pay-grid">
        <div class="spay"><div class="spay-sym">&#x1F3AF;</div><div class="spay-info"><div class="spay-n">Цель &#xD7;3</div><div class="spay-m">x 50</div></div></div>
        <div class="spay"><div class="spay-sym">&#x1F525;</div><div class="spay-info"><div class="spay-n">Огонь &#xD7;3</div><div class="spay-m">x 30</div></div></div>
        <div class="spay"><div class="spay-sym">&#x1F48E;</div><div class="spay-info"><div class="spay-n">Бриллиант &#xD7;3</div><div class="spay-m">x 20</div></div></div>
        <div class="spay"><div class="spay-sym">&#x1F3C6;</div><div class="spay-info"><div class="spay-n">Трофей &#xD7;3</div><div class="spay-m">x 10</div></div></div>
        <div class="spay"><div class="spay-sym">&#x26A1;</div><div class="spay-info"><div class="spay-n">Энергия &#xD7;3</div><div class="spay-m">x 6</div></div></div>
        <div class="spay"><div class="spay-sym">&#x1F94A;</div><div class="spay-info"><div class="spay-n">Перчатки &#xD7;3</div><div class="spay-m">x 5</div></div></div>
        <div class="spay"><div class="spay-sym">&#x1F3CB;</div><div class="spay-info"><div class="spay-n">Штанга &#xD7;3</div><div class="spay-m">x 4</div></div></div>
        <div class="spay"><div class="spay-sym">&#x1F4AA;</div><div class="spay-info"><div class="spay-n">Бицепс &#xD7;3</div><div class="spay-m">x 3</div></div></div>
        <div class="spay spay-2"><div class="spay-sym">&#x2728;</div><div class="spay-info"><div class="spay-n">Любые 2 одинаковых</div><div class="spay-m">x 1.5</div></div></div>
      </div>
    </div>
  </div>
</div>
...
<nav id="bnav">
  ...
  <button class="bnb" data-t="slots">
    <span class="bnb-i">🎰</span>
    <span class="bnb-l">Слоты</span>
  </button>
...
function doClick(x,y){
  ...
  // Coin tap sound
  var _ac=slotsAudioCtx();
  if(_ac&&SLOTS.soundOn){
    try{
      if(_ac.state==='suspended')_ac.resume();
      var _o=_ac.createOscillator(),_g=_ac.createGain();
      _o.connect(_g);_g.connect(_ac.destination);
      _o.type='sine';
      _o.frequency.setValueAtTime(500,_ac.currentTime);
      _o.frequency.exponentialRampToValueAtTime(220,_ac.currentTime+0.07);
      _g.gain.setValueAtTime(0.1,_ac.currentTime);
      _g.gain.exponentialRampToValueAtTime(0.001,_ac.currentTime+0.07);
      _o.start(_ac.currentTime);_o.stop(_ac.currentTime+0.07);
    }catch(e){}
  }
...
function crashGenPt(){var r=Math.random();if(r<.50)return 1.01+Math.random()*.29;if(r<.75)return 1.30+Math.random()*.70;if(r<.90)return 2.0+Math.random()*2;if(r<.97)return 4+Math.random()*6;return 10+Math.random()*15;}
...
// ─── SLOTS ────────────────────────────────────────────────────────────────────
var SLOT_SYMS=[
  {e:'💪',n:'Бицепс',  pay3:3,  w:35},
  {e:'🏋️',n:'Штанга',  pay3:4,  w:25},
  {e:'🥊',n:'Перчатки',pay3:5,  w:18},
  {e:'⚡',n:'Энергия', pay3:6,  w:14},
  {e:'🏆',n:'Трофей',  pay3:10, w:10},
  {e:'💎',n:'Бриллиант',pay3:20,w:6},
  {e:'🔥',n:'Огонь',   pay3:30, w:4},
  {e:'🎯',n:'Цель',    pay3:50, w:2}
];
var SLOTS={bet:0,spinning:false,soundOn:true,_ctx:null};

function slotsAudioCtx(){
  if(!SLOTS._ctx){try{SLOTS._ctx=new(window.AudioContext||window.webkitAudioContext)();}catch(e){}}
  return SLOTS._ctx;
}
function playSlotSound(type){
  if(!SLOTS.soundOn)return;
  var ctx=slotsAudioCtx();if(!ctx)return;
  function tone(freq,endFreq,dur,vol,shape){
  ...
  if(type==='spin'){
    for(var i=0;i<12;i++){(function(d){setTimeout(function(){tone(150+Math.random()*300,null,.05,.06,'square');},d);})(i*60);}
  } else if(type==='reel'){
    tone(320,140,.12,.25,'sine');
  } else if(type==='tick'){
    tone(600,null,.03,.04,'square');
  } else if(type==='win'){
    [440,554,659,880].forEach(function(f,i){setTimeout(function(){tone(f,null,.28,.2,'sine');},i*100);});
  } else if(type==='jackpot'){
    [440,554,659,880,1100,880,1100,1320].forEach(function(f,i){setTimeout(function(){tone(f,null,.22,.25,'triangle');},i*75);});
  } else if(type==='lose'){
    tone(280,80,.45,.12,'sawtooth');
  }
}
function slotRand(){
  var t=SLOT_SYMS.reduce(function(s,x){return s+x.w;},0),r=Math.random()*t;
...
function setSlotBet(val){
  ...
  document.querySelectorAll('.sbb').forEach(function(btn){btn.classList.toggle('sel',btn.dataset.sbet==val);});
}
function doSpin(){
  if(SLOTS.spinning)return;
  if(SLOTS.bet<=0){showNotif('Сделай ставку!');return;}
  if(G.coins<SLOTS.bet){showNotif('Недостаточно бицушек!');return;}
  // Resume AudioContext if suspended (mobile browsers require user gesture)
  var actx=slotsAudioCtx();if(actx&&actx.state==='suspended')actx.resume();
  G.coins-=SLOTS.bet;updateHUD();
  SLOTS.spinning=true;
  document.getElementById('slotSpinBtn').disabled=true;
  document.getElementById('slotsResult').innerHTML='';
  for(var r=1;r<=3;r++){var el=document.getElementById('slotReel'+r);if(el){el.classList.remove('win');el.classList.add('spinning');}}
  playSlotSound('spin');
  var results=[slotRand(),slotRand(),slotRand()];
  var stopTimes=[1100,1900,2700];
  ...
  for(var ri=0;ri<3;ri++){
    (function(idx){
      ...
      var iv=setInterval(function(){
        var s=SLOT_SYMS[Math.floor(Math.random()*SLOT_SYMS.length)];
        var el=document.getElementById('slotReel'+(idx+1));
        if(el)el.querySelector('.slot-symbol').textContent=s.e;
        tickCnt++;if(tickCnt%4===0)playSlotSound('tick');
      },75);
      spinInts.push(iv);
      setTimeout(function(){
        clearInterval(iv);
        var reelEl=document.getElementById('slotReel'+(idx+1));
        if(reelEl){reelEl.classList.remove('spinning');reelEl.querySelector('.slot-symbol').textContent=SLOT_SYMS[results[idx]].e;}
        playSlotSound('reel');
        stopped[idx]=true;
        if(stopped[0]&&stopped[1]&&stopped[2])setTimeout(function(){slotCheckWin(results);},350);
      },stopTimes[idx]);
    })(ri);
  }
}
function slotCheckWin(res){
  var bet=SLOTS.bet,win=0,msg='';
  if(res[0]===res[1]&&res[1]===res[2]){
    var sym=SLOT_SYMS[res[0]],mult=sym.pay3;
    win=Math.floor(bet*mult);
    for(var r=1;r<=3;r++){var el=document.getElementById('slotReel'+r);if(el)el.classList.add('win');}
    if(mult>=30){playSlotSound('jackpot');msg='<span style="color:#ffd700;font-size:15px">\uD83C\uDF89 ДЖЕКПОТ! '+sym.e+sym.e+sym.e+' \xD7'+mult+' = +'+fmt(win)+' \uD83D\uDCAA</span>';}
    else{playSlotSound('win');msg='<span style="color:var(--gn)">\u2705 ПОБЕДА! '+sym.e+sym.e+sym.e+' \xD7'+mult+' = +'+fmt(win)+' \uD83D\uDCAA</span>';}
  } else if(res[0]===res[1]||res[1]===res[2]||res[0]===res[2]){
    win=Math.floor(bet*1.5);playSlotSound('win');
    msg='<span style="color:var(--bl)">\u2728 Два совпадения! \xD71.5 = +'+fmt(win)+' \uD83D\uDCAA</span>';
    if(res[0]===res[1]){document.getElementById('slotReel1').classList.add('win');document.getElementById('slotReel2').classList.add('win');}
    else if(res[1]===res[2]){document.getElementById('slotReel2').classList.add('win');document.getElementById('slotReel3').classList.add('win');}
    else{document.getElementById('slotReel1').classList.add('win');document.getElementById('slotReel3').classList.add('win');}
  } else {
    playSlotSound('lose');
    msg='<span style="color:var(--mt)">\uD83D\uDE14 Не повезло... \u2212'+fmt(bet)+' \uD83D\uDCAA</span>';
  }
  if(win>0){G.coins+=win;G.allC+=win;updateHUD();chkAchs();showNotif('+'+fmt(win)+' бицушек!');}
  document.getElementById('slotsResult').innerHTML='<div class="slots-res-txt">'+msg+'</div>';
...
window.addEventListener('DOMContentLoaded',function(){
  ...
  if(coinScene){
    coinScene.addEventListener('touchstart',function(e){
      e.preventDefault();
      var ac=slotsAudioCtx();if(ac&&ac.state==='suspended')ac.resume();
  ...
  // ─── СЛОТЫ — инициализация ────────────────────────────────────────────────
  document.querySelectorAll('.sbb').forEach(function(btn){
    btn.addEventListener('click',function(){setSlotBet(this.dataset.sbet);});
  });
  document.getElementById('slotSpinBtn').addEventListener('click',doSpin);
  document.getElementById('slotVolBtn').addEventListener('click',function(){
    SLOTS.soundOn=!SLOTS.soundOn;
    this.classList.toggle('on',SLOTS.soundOn);
    this.innerHTML=SLOTS.soundOn?'&#x1F50A;':'&#x1F507;';
  });
});
</script>
</body>
