...
.crash-mult.danger{color:var(--rd);text-shadow:0 0 30px rgba(255,34,68,.6);animation:dngr .1s infinite}
@keyframes dngr{0%,100%{transform:translateX(0)}50%{transform:translateX(3px)}}
...
.crash-bar{width:100%;max-width:280px;height:6px;background:rgba(255,255,255,.07);border-radius:3px;overflow:hidden}
.crash-barf{height:100%;background:linear-gradient(90deg,var(--gn),var(--gd));border-radius:3px;transition:width .1s linear}
.crash-barf.d{background:linear-gradient(90deg,var(--or),var(--rd))}
...
<div class="panel" id="panel-passive">
  <div class="sec">Инвестиции в страны</div><div id="lPassive"></div>
</div>
...
<div class="panel" id="panel-crash">
  <div class="crash-wrap">
    ...
    <div class="crash-screen">
      ...
      <div class="crash-bar"><div class="crash-barf" id="crashFill" style="width:0%"></div></div>
...
<div class="panel" id="panel-coin">
  <div class="coin-soon-wrap">
    ...
    <a class="coin-soon-btn" href="https://t.me/gymclicker" target="_blank">&#x1F4E2; Подписаться</a>
  </div>
</div>
<div id="notif"></div>
...
var SAVE_KEY='gymv12';
var DEF={coins:0,allC:0,clicks:0,allU:0,crits:0,combos:0,crashWins:0,lvl:1,xp:0,nrg:100,mxE:100,rgE:2,cpc:1,cps:0,critC:0,critM:2,luck:0,combo:0,pt:0,mxCps:0,mxCpc:1,ul:{},achs:[],claimed:[],sk:['default'],skin:'default',lastSeen:null,comboN:0};
var G={};
var TIMERS={};
...
function buyUpgrade(id){
  ...
  if(TIMERS[id]&&!TIMERS[id].done){
    ...
    recalc();chkLvl();chkAchs();updateHUD();
    delete TIMERS[id];
    renderAll();showNotif(u.n+' — готово! Ур.'+uLvl(id));
    return;
  }
...
function applySkin(id){
  ...
  btn.style.background=s.bg;btn.style.boxShadow=s.sh;btn.innerHTML=s.e;
...
function spawnFlt(txt,x,y,col){var el=document.createElement('div');el.className='ft';el.innerHTML=txt;if(col)el.style.color=col;el.style.left=(x+(Math.random()*40-20))+'px';el.style.top=(y-10)+'px';document.body.appendChild(el);setTimeout(function(){el.remove();},900);}
...
function crashGenPt(){var r=Math.random();if(r<.40)return 1+Math.random()*.5;if(r<.65)return 1.5+Math.random();if(r<.80)return 2.5+Math.random()*2.5;if(r<.92)return 5+Math.random()*10;if(r<.98)return 15+Math.random()*35;return 50+Math.random()*150;}
...
function crashStart(){
  ...
  CRASH.timer=setInterval(function(){
    CRASH.mult=parseFloat((CRASH.mult+spd).toFixed(2));spd=Math.min(spd+0.001,0.15);
    var pct=Math.min(100,(CRASH.mult/CRASH.crashAt)*100);
    var el=document.getElementById('crashMult');el.textContent='x'+CRASH.mult.toFixed(2);
    document.getElementById('crashFill').style.width=pct+'%';
    var danger=CRASH.mult>CRASH.crashAt*0.75;
    el.classList.toggle('danger',danger);
    document.getElementById('crashFill').classList.toggle('d',danger);
...
function crashEnd(won){
  ...
  setTimeout(function(){
    ...
    document.getElementById('crashFill').classList.remove('d');
...
window.addEventListener('DOMContentLoaded',function(){
  loadGame();recalc();applySkin(G.skin||'default');updateHUD();renderAll();chkOffline();chkNick();renderCrashHist();
  setInterval(tick,100);setInterval(saveGame,5000);setInterval(renderAll,1000);setInterval(chkAchs,3000);setInterval(pushScore,30000);
  ...
  document.getElementById('tabs').addEventListener('click',function(e){
    var b=e.target.closest('.tab');if(!b)return;var t=b.dataset.t;
  ...
  (function(){
    ...
    var la=document.createElement('button');la.className='tarr tl';la.textContent='<';
    var ra=document.createElement('button');ra.className='tarr tr';ra.textContent='>';
    wrap.appendChild(la);wrap.appendChild(ra);
    function upd(){la.classList.toggle('show',tabs.scrollLeft>5);ra.classList.toggle('show',tabs.scrollLeft<tabs.scrollWidth-tabs.clientWidth-5);}
    tabs.addEventListener('scroll',upd);upd();setTimeout(upd,500);
    la.addEventListener('click',function(){tabs.scrollBy({left:-130,behavior:'smooth'});});
    ra.addEventListener('click',function(){tabs.scrollBy({left:130,behavior:'smooth'});});
  })();
